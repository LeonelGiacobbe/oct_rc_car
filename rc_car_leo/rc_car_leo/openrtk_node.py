...
class RTKINS(MsgSpec):
    ...
    def __call__(self, raw_data : bytes):
        ...
        msg.pose.covariance = \
            [data.lat_std] + [0]*6 + [data.lon_std] + [0]*6 + [data.alt_std] + [0]*6 + \
            [data.pitch_std]+[0]*6 + [data.roll_std]+ [0]*6 + [data.heading_std]

        # velocity is tricky as it's supplied ENU and needs to be wrt vehicle coords
        # solution: pack into twist then transform it

        msg.twist.twist.x = data.east_velocity
        msg.twist.twist.y = data.north_velocity
        msg.twist.twist.z = data.up_velocity
        msg.twist.covariance = [data.east_vel_std] + [0]*3 + [data.north_vel_std] + [0]*3 + [data.up_vel_std]
    ...
    def rotate_twist(twist, quat):
        ...
        vels = np.dot(rot, np.array([twist.twist.x, twist.twist.y, twist.twist.z]).reshape((3,1)))
        ...
        ret.covariance = v_cov[0:3] + [0]*3 + v_cov[3:6] + [0]*3 + v_cov[6:9] + [0]*21
...
class OpenRTK(rclpy.node.Node):
    def __init__(self):
        ...
        self.msgs = [
            ...
            RTKINS(self.create_publisher(Odometry, 'odometry', 10), self.get_clock())
        ]
    ...
    def run(self):
        ...
        with serial.Serial(
            ...
            self.get_parameter('uart_buadrate').get_parameter_value().integer_value,
            timeout=self.get_parameter('uart_timeout').get_parameter_value().float_value
        ) as port:
            ...
            while rclpy.ok():
                ...
                if port.read(1) == b'\x55' and port.read(1) == b'/x55':
                    ...
                    if any(map(self.msgs, key=lambda x: repr(x)==frame_type)):
                        # now data length
                        data_len = int(port.read(1))
                        data = port.read(data_len)
                        checksum = port.read(2)
                        # TODO validate checksum
                        if data_len == len(data) or self.validate_checksum(data, checksum):
                            for parser in filter(lambda x: repr(x)==frame_type, self.msgs):
                                # parse data and pack into message
                                parser(data)

                    else:
                        # report the bad frame type
                        self.get_logger().info(f"Invalid frame type received: {''.join([hex(x) for x in frame_type])}")
